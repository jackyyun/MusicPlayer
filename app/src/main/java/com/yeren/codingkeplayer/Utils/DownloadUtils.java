...
import com.yeren.codingkeplayer.MainActivity;
        ...
        mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    System.out.println("yeren tag0"+url);
    ...
    private void downloadLRC(final String url,final String musicName, final Handler handler){
        mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                Document doc = null;
                try {
                    doc = Jsoup.connect(url).userAgent(Constant.USER_AGENT).timeout(6000).get();
